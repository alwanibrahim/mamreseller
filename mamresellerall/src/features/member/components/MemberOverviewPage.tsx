...
type MemberOverview = {
  totals: { orders: number; spent: number; topup: number; balance: number; fulfillment: number };
  daily: { date: string; orders: number; spend: number }[];
  byProduct: { name: string; orders: number }[];
...
export function MemberOverviewPage() {
  ...
  useEffect(() => {
    apiFetch<MemberOverview>("/api/member/overview")
      .then(setData)
      .catch((e) => setError(e instanceof Error ? e.message : "failed to load overview"));
  }, []);

  const pieConfig = Object.fromEntries(
    (data?.byProduct ?? []).map((p, i) => [p.name, { label: p.name, color: pieColors[i % pieColors.length] }]),
  ) satisfies ChartConfig;
      ...
      {data && (
        <>
          <div className="grid gap-4 sm:grid-cols-4">
            {[
              { label: "Balance", value: fmtUSD.format(data.totals.balance) },
              { label: "Orders", value: String(data.totals.orders) },
              { label: "Total Spent", value: fmtUSD.format(data.totals.spent) },
          ...
          <div className="grid gap-4 md:grid-cols-2">
              ...
              <CardContent>
                <ChartContainer config={areaConfig} className="h-64 w-full">
                  <AreaChart data={data.daily} margin={{ left: 12, right: 12 }}>
                    ...
                    <Area dataKey="spend" type="natural" fill="var(--color-spend)" fillOpacity={0.2} stroke="var(--color-spend)" />
                  </AreaChart>
                </ChartContainer>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Your Orders per Product</CardTitle>
                <CardDescription>Top products you bought</CardDescription>
              </CardHeader>
              <CardContent>
                {data.byProduct.length === 0 ? (
                  ...
                  <ChartContainer config={pieConfig} className="mx-auto h-64 w-full">
                    <PieChart>
                      ...
                      <Pie data={data.byProduct} dataKey="orders" nameKey="name" innerRadius={50} strokeWidth={4}>
                        {data.byProduct.map((p, i) => (
                          <Pie key={p.name} data={[p]} dataKey="orders" nameKey="name" fill={pieColors[i % pieColors.length]} />
                        ))}
                      </Pie>
                    </PieChart>
                  </ChartContainer>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Your Orders by Day</CardTitle>
                <CardDescription>Which day of the week you order</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Your delivered orders</CardDescription>
              </CardHeader>
              <CardContent>
