// Konektor Excalibur — mengikuti kontrak response.md
// Upstream: X-API-Key, path /api/v1/*, pagination page-based, tanpa idempotency upstream (dedup lokal)
...
// created_at tanpa zona (excalibur) → diasumsikan UTC
function toISO(s: unknown, assumeOffset = "Z"): string | null {
  if (!s) return null;
...
// 400 upstream bisa "Insufficient balance" atau "stok kurang" → bedakan via pesan
